...
export const docsTerminalBlock = defineBlock({
  ...
  props: {
    "title": fields.singleLine({ label: "Window Title", placeholder: "e.g., Terminal" }),
    "commands": fields.repeater({ label: "Commands", itemSchema: {
      "prompt": fields.singleLine({ label: "Prompt", defaultValue: "$" }),
      "command": fields.singleLine({ label: "Command", required: true }),
      "output": fields.multiLine({ label: "Output" })
    } }),
